<template>
    <main class="main">
        <div class="main__about">
            <h1 class="title-text">{{episode.name}}</h1>
            <h2 class="subtitle-text">{{episode.episode}}</h2>
...
n
        ...
        <div class="main__fieldWrapper">
            ...
            <div v-for="character in filteredItems" :key="character.id" class="charactersField__card">
                <div class="charactersField__card">
...
export default {
    ...
    mounted(){
        return axios.get(`https://rickandmortyapi.com/api/episode/${this.id}`)
        .then (response => {
            this.episode = response.data,
            response.data.characters.map(item => axios.get(item).then(res => this.CharacterData.push(res.data)))
        })
        .catch(err => {
        console.log(err)
        })
